...
func Signup() gin.HandlerFunc {
	...
	return func(c *gin.Context){
		...
		defer cancel()   // This will call cancel() when the main function exits. This ensures that the context is properly canceled, which can signal other parts of your program to clean up and terminate gracefully.
		...
		user.Created_At, _ = time.Parse(timeRFC3339, time.Now().Format(time.RFC3339))
		user.Updated_At, _ = time.Parse(timeRFC3339, time.Now().Format(time.RFC3339))
